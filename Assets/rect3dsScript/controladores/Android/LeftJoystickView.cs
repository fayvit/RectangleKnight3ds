...
public class LeftJoystickView : MonoBehaviour
{
    [SerializeField] private ControladorDeJoystick leftJoystick;
            ...
            for (int i = 0; i < Input.touchCount; i++)
            {
                ...
                if (myTouches[i].phase == TouchPhase.Began)
                {
                    ...
                    if (myTouches[i].position.x < Screen.width / 2)
                    {
                        ...
                        if (leftJoystick.joystickComPosicaoFixa == false)
                        {
                            Debug.Log("a iniciação");
                ...
                if (myTouches[i].phase == TouchPhase.Ended)
                {
                    ...
                    {
                        StartCoroutine(TestHideController(myTouches[i]));
                    }
                }
            }
        }
    ...
    IEnumerator TestHideController(Touch t)
    {
        yield return new WaitForSeconds(0.25f);

        Debug.Log("phase aqui: "+t.phase);
        if(t.fingerId==leftSideFingerID && t.phase== TouchPhase.Ended)
        {
            Debug.Log("finalização da dedada");
